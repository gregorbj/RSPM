...
for( yr in RunYears ) {
  ...
  for( md in Md ) {
    ...
    for( dx in Dx ) {
      IsDistrict. <- SynPop..$District == dx
      if( any( IsDistrict. ) ){
        SynPop..$Carshare[IsDistrict.] <- idCarshareHh( SynPop..[ IsDistrict., ModelVar. ],
                                                        CarshareRate=CarshareRates.DiYr[dx,yr] )
      }
    }
    rm( ModelVar. )
    ...
    ModelVar. <- c( "Hhincttl", "Htppopdn", "Hhvehcnt", "Tranmilescap", "Fwylnmicap", "DrvAgePop", "Hhsize", 
                    "Age0to14", "Age15to19", "Age20to29", "Age30to54", "Age55to64", "Age65Plus", "Urban", 
                    "BaseCostPerMi", "FutrCostPerMi" )
    ...
    SynPop..$Dvmt <- 0
    SynPop..$Dvmt <- calcAdjAveDvmt( SynPop..[ , ModelVar. ], DvmtLmModels_, "Metro", 
    ...
    SynPop..$MaxDvmt <- 0
    SynPop..$Dvmt95 <- 0
    MetroMax95th.2d <- predictMaxDvmt( SynPop..[ , c( "Dvmt", "MaxDvmt", "Dvmt95" ) ],
                                       DvmtLmModels_, "Metro" )
    SynPop..$MaxDvmt <- MetroMax95th.2d[,1]
    SynPop..$Dvmt95 <- MetroMax95th.2d[,2]
    gc()
    ...
    #Step 2g: Apply parking model to identify parkers and calculate daily parking costs
    #==================================================================================
  ...
  for( md in Md ) {
    ...
    SynPop..$Payd <- selectFromWeights(PaydWeights.Hh, PropToSelect=Payd..Yr[yr, "Proportion"])
  ...
  for( it in It ) {
    ...
    for( md in Md ) {
      ...
      Dvmt.MdDt[md,] <- tapply( SynPop..$Dvmt, SynPop..$DevType, sum )[Dt]
      ...
      CostSummary.MdVa[ md, "FutrCostPerMi" ] <- sum( Costs_$FutrCostPerMi )
      ...
      ModelVar. <- c( "Hhincttl", "CashOutIncAdj", "Htppopdn", "Hhvehcnt", "Tranmilescap", "Fwylnmicap", "DrvAgePop", 
      ...
      # Tabulate DVMT
      #--------------
      Dvmt.MdDt[ md, ] <- tapply( SynPop..$Dvmt, SynPop..$DevType, sum, na.rm=TRUE )[Dt]
      Dvmt.MdDt[ is.na( Dvmt.MdDt ) ] <- 0
    ...
    local( {
      ...
      #-----------------------------
      Dvmt.Ty <- c( LtVehDvmt, TruckDvmt, BusDvmt )
    ...
    LtVehDvmt <- sum( CostSummary.MdVa[ , "AdjDvmt" ] ) + sum( CommServ_$CommServDvmt.MdDt )
